...
interface SearchRecord {
  ...
  line: number
  from: number
  mediaKind: 'binary' | 'image' | 'opt' | 'video' | 'zip' | null
}
      ...
      <Dialog.Content className="search-dialog" aria-describedby={undefined}>
        ...
        <div className="global-search-results">
          {results.length ? results.map((record, index) => <button key={record.id} className={index === selected ? 'selected' : ''} onPointerMove={() => setSelected(index)} onClick={() => choose(record)}>
            {record.mediaKind === 'image' ? <FileImage size={20} /> : record.mediaKind === 'video' ? <FileVideo size={20} /> : record.mediaKind === 'zip' || record.mediaKind === 'opt' ? <Archive size={20} /> : <FileText size={20} />}
